...
nginx_conf_template = "./files/etc/nginx/conf.d/default.conf.template"
nginx_conf_file = "./files/etc/nginx/conf.d/default.conf"
ssl_params_template = "./files/etc/nginx/ssl_params.template"
ssl_params_file = "./files/etc/nginx/ssl_params"

def generate_compose_file(data_source):

    with open(compose_yml_template, "r") as stream:
...
def generate_nginx_conf(data_source):
    ...
    conf_str = open(nginx_conf_template, "r").read()
    if srv_role == "reverse-proxy":
    ...
    open(nginx_conf_file, "w").write(conf_str)
    
    if is_tls == True:
        ssl_params_str = open(ssl_params_template, "r").read()
        ssl_params_str = ssl_params_str.replace("${SERVER_NAME}", server_name)
        open(ssl_params_file, "w").write(ssl_params_str)
     
def main():

    with open(data_source_file, "r") as stream:
        data_source = yaml.safe_load(stream)

    generate_compose_file(data_source)
    generate_nginx_conf(data_source)
